 ...
 * Locales entities. Transaction control of the save(), update() and delete()
 * operations must be handled externally by senders of these methods or must be
 * manually added to each of these methods for data to be persisted to the JPA
 * datastore.
 * 
 * @see cl.intelidata.amicar.bd.Locales
...
public class LocalesDAO implements ILocalesDAO {
	...
	public void save(Locales entity) {
		EntityManagerHelper.log("saving Locales instance", Level.INFO, null);
		try {
	...
	public void delete(Locales entity) {
		EntityManagerHelper.log("deleting Locales instance", Level.INFO, null);
		try {
			entity = getEntityManager().getReference(Locales.class,
					entity.getIdLocal());
	 ...
	 * <pre>
	 * EntityManagerHelper.beginTransaction();
	 * entity = LocalesDAO.update(entity);
	 * EntityManagerHelper.commit();
	 * </pre>
	 * 
	 * @param entity
	...
	public Locales update(Locales entity) {
		EntityManagerHelper.log("updating Locales instance", Level.INFO, null);
		try {
			Locales result = getEntityManager().merge(entity);
    ...
    public Locales findById(Integer id) {
		EntityManagerHelper.log("finding Locales instance with id: " + id,
				Level.INFO, null);
		try {
			Locales instance = getEntityManager().find(Locales.class, id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Locales> findByProperty(String propertyName,
			final Object value, final int... rowStartIdxAndCount) {
		EntityManagerHelper.log("finding Locales instance with property: "
				+ propertyName + ", value: " + value, Level.INFO, null);
		try {
			final String queryString = "select model from Locales model where model."
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<Locales> findAll(final int... rowStartIdxAndCount) {
		EntityManagerHelper.log("finding all Locales instances", Level.INFO,
				null);
		try {
			final String queryString = "select model from Locales model";
			Query query = getEntityManager().createQuery(queryString);
			if (rowStartIdxAndCount != null && rowStartIdxAndCount.length > 0) {
				int rowStartIdx = Math.max(0, rowStartIdxAndCount[0]);
				if (rowStartIdx > 0) {
